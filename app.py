...
@app.route("/api/enroll/add-round", methods=["POST"])
def api_enroll_add_round():
    """Voluntary post-enrollment growth: after a genuine, accepted login,
    the user can choose to add that same attempt as an EXTRA enrollment
    round. Unlike the always-on adaptive EMA update in api_login_verify
    (which only nudges the existing mean/std), this stores the full raw
    sample and rebuilds the profile from the complete history — the more
    samples behind a profile, the tighter and more reliable it is (see
    reliability_report.py: FAR/FRR both improve with more real samples
    per user). Requires the password again and re-runs the bot check, so
    this can't be used to poison a profile with scripted input."""
    data = _json_body()
    if data is None:
        return _bad("Invalid request body.")
    user = db.get_user(_username(data))
    ...
    if not user["enrolled"]:
        return _bad("Complete initial enrollment before adding extra rounds.", 409)
    if not _password_ok(user, _password(data)):
    ...
    if is_bot:
        return _bad("This attempt looked automated and can't be added to your baseline.",
                     400, bot_reasons=bot_reasons)

    existing = db.get_enrollment_samples(user["id"])
    round_index = len(existing) + 1
    db.add_enrollment_sample(user["id"], round_index, features)

    all_samples = existing + [features]
    profile = be.build_profile(all_samples)
    db.save_profile(user["id"], profile, len(all_samples))

    return jsonify(ok=True, round=round_index, sample_count=len(all_samples))
