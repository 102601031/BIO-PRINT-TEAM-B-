...
def score_attempt(profile, features, weights=None):
    """
    Returns (confidence 0-100, per_feature list of dicts sorted by
    how much each feature deviated from baseline).

    `weights` optionally overrides FEATURE_WEIGHTS (e.g. with data-driven
    weights from reliability_report.py's discriminability analysis).
    """
    if not profile:
        return 0.0, []

    weights = weights if weights is not None else FEATURE_WEIGHTS
    breakdown = []
    weighted_sq_sum = 0.0
    weight_sum = 0.0
    for k, stats in profile.items():
